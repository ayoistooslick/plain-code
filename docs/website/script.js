...
var LIBRARY = [
  ...
  {cat:"cli",name:"CLI: new",desc:"Scaffold a new PlainScript project",ps:'plainscript new my-app',js:'// Creates my-app/ with src/app.ps and package.json',skipRun:true},
  {cat:"cli",name:"CLI: run",desc:"Compile and execute a PlainScript file",ps:'plainscript run src/app.ps',js:'// Compiles src/app.ps and runs the output',skipRun:true},
  {cat:"cli",name:"CLI: build",desc:"Compile .ps files to JavaScript",ps:'plainscript build',js:'// Compiles src/app.ps to dist/app.js',skipRun:true},
  {cat:"cli",name:"CLI: fmt",desc:"Auto-format PlainScript code",ps:'plainscript fmt src/app.ps',js:'// Formats src/app.ps in place',skipRun:true},
  ...
  {cat:"cli",name:"CLI: check",desc:"Type-check without emitting output",ps:'plainscript check src/app.ps',js:'// Validates syntax without compiling',skipRun:true},
  {cat:"cli",name:"CLI: watch",desc:"Watch files and recompile on change",ps:'plainscript watch',js:'// Starts file watcher for .ps files',skipRun:true},
  ...
  {cat:"cli",name:"CLI: version",desc:"Print the PlainScript version",ps:'plainscript version',js:'// Prints v1.0.0-latest',skipRun:true},
  ...
  {cat:"functions",name:"Default Parameters",desc:"Provide fallback values for function arguments",ps:'make greet(name, greeting as "Hello")\n  show `${greeting}, ${name}!`\ndone\n\ngreet("Ayo")\ngreet("Ayo", "Hey")',js:'function greet(name, greeting = "Hello") {\n  console.log(`${greeting}, ${name}!`);\n}\n\ngreet("Ayo");\ngreet("Ayo", "Hey");'},
  ...
  {cat:"concurrency",name:"All Of (Promise.all)",desc:"Await every promise and collect results",ps:'make fetchA()\n  give "a"\ndone\nmake fetchB()\n  give "b"\ndone\nremember both as all of [fetchA(), fetchB()]\nshow both',js:'let both = await Promise.all([fetchA(), fetchB()]);\nconsole.log(both);'},
  {cat:"concurrency",name:"Any Of (Promise.race)",desc:"Resolve with the first promise to settle",ps:'make slow()\n  give "slow"\ndone\nmake fast()\n  give "fast"\ndone\nremember winner as any of [slow(), fast()]\nshow winner',js:'let winner = await Promise.race([slow(), fast()]);\nconsole.log(winner);'},
  {cat:"concurrency",name:"Settled Of (Promise.allSettled)",desc:"Await all promises; each yields status/value/reason",ps:'make ok(x)\n  give x\ndone\nremember s as settled of [ok(1), ok(2)]\nshow s[0].status\nshow s[1].value',js:'let s = await Promise.allSettled([ok(1), ok(2)]);\nconsole.log(s[0].status);\nconsole.log(s[1].value);'},
  {cat:"concurrency",name:"With Timeout",desc:"Reject a promise if it does not settle in time",ps:'make job()\n  sleep(500)\n  give "done"\ndone\nremember r as withTimeout(job(), 100)\nshow "finished"',js:'let r = await __withTimeout(job(), 100);\nconsole.log("finished");'},
  {cat:"async",name:"Await (wait for)",desc:"Await a promise to get its resolved value",ps:'make fetchData()\n  give 42\ndone\nremember value as wait for fetchData()\nshow value',js:'let value = (await fetchData());\nconsole.log(value);'},
  ...
  {cat:"generators",name:"Yield Generator",desc:"yield inside make makes a generator",ps:'make countUp(n)\n  remember i as 0\n  while i is less than n\n    i becomes i + 1\n    yield i\n  done\ndone\nshow spread of countUp(3)',js:'function* countUp(n) {\n  let i = 0;\n  while (i < n) { i = i + 1; yield i; }\n}\nconsole.log([...countUp(3)]);'},
