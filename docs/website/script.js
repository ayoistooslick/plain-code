...
var LIBRARY = [
  ...
  {cat:"data",name:"Dynamic Property Access",desc:"Use a variable as an object key",ps:'remember key as "name"\nremember user as {name: "Ayo", age: 25}\nremember value as the [key] of user\nshow value',js:'let key = "name";\nlet user = { "name": "Ayo", "age": 25 };\nlet value = user[key];\nconsole.log(value);'},
  {cat:"async",name:"Parallel Async",desc:"Run multiple async operations concurrently",ps:'wait for all fetch("api/users"), fetch("api/posts") as results\nshow results',js:'const results = await Promise.all([fetch("api/users"), fetch("api/posts")]);\nconsole.log(results);'},
