// Stream-language spec for the PlainScript programming language.
//
// Pure CommonJS with no external dependencies so it can be:
//  1. unit-tested by tests/compiler.test.js (plain Node.js), and
//  2. wrapped by plainscript-acode/main.js in @codemirror/language's StreamLanguage.
//
...
// These lex as identifiers/keywords but only have meaning inside PlainScript's
...
// PlainScript-language tokenizer. Runs at the start of a token; must always
...
function token(stream, state) {
  ...
  // Handle the closing `}` here, then fall through to PlainScript tokenization
  ...
  if (state.inTemplateExpr) {
    ...
    // Fall through below for normal PlainScript tokenization of the inner expression.
...
// and returns to PlainScript (mirrors compiler/lexer.js raw-block collection).
...
// it stays visually distinct from PlainScript code; the block ends at `done`.
...
function tokenTemplate(stream, state) {
  // Inside a `${...}` expression — PlainScript tokenization is handled by the
...
// Exported spec consumed by plainscript-acode/main.js (StreamLanguage.define).
module.exports = {
  name: 'plainscript',
  startState: () => ({
    inJavaScript: false,
